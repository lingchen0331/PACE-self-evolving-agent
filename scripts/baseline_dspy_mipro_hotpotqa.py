#!/usr/bin/env python3
"""
DSPy MIPROv2 baseline for HotpotQA (distractor setting).

Usage:
    source .venv/bin/activate
    python scripts/baseline_dspy_mipro_hotpotqa.py [--model qwen3-local] [--auto medium]
...
# ── CLI ──────────────────────────────────────────────────────────────
def parse_args():
    p = argparse.ArgumentParser(description="DSPy MIPROv2 baseline on HotpotQA")
    ...
    p.add_argument("--output-dir", default="results/dspy_mipro_hotpotqa")
    return p.parse_args()
...
def load_hotpotqa_splits(train_n, val_n, test_n, seed=0):
    ...
    # train split -> MIPROv2 train + val; validation split -> test
...
# ── Metric ───────────────────────────────────────────────────────────
def hotpotqa_metric(gold, pred, trace=None):
