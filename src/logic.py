...
def store_all_logic(dump_folder='../dumped_agent'):
    ...
    all_tasks = []
    # Iterate over snapshots to avoid RuntimeError when module/global dicts mutate during traversal.
    ...
    failed = [result for result in results if result.startswith("Failed to dump ")]
    # merge_and_clean(dump_folder)
    dump_folder_display = dump_folder if str(dump_folder).endswith("/") else f"{dump_folder}/"
    print(f"Dumped logics to {dump_folder_display}")
    if failed:
        print(f"Skipped {len(failed)} objects while dumping.")
